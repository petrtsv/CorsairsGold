...
import site.petrtsv.corsairs.screens.InfoScreen;

/**
 * Created by Петр on 17.08.2017.
 * <p>
 * Button, that open the InfoScreen.
 */
...
public class InfoButton extends GameButton
{
	...
	public InfoButton(UIGroup group, @SuppressWarnings("SameParameterValue") int x, int y)
	{
		super(group, x, y);
		usualRegion = TextureManager.getInstance().getTextureRegion("help_button_notpressed");
		pressedRegion = TextureManager.getInstance().getTextureRegion("help_button_pressed");
	...
	public void onTouchUp(int pointer)
	{
		if (this.pointer == pointer)
		{
			...
			Screen newScreen = new InfoScreen(screen.getGame(),
